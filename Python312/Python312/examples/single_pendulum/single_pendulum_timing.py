...
------------
    ...
    This example is the same as single_pendulum.py, except that it displays 
    information in the pybox debug/Process control window showing how many milliseconds
    it takes to calculate and render the image between vertical retraces.

    This is meant to show a simple version of using some of the pybox drawing functions in 
    a real-time setting.

    see single_pendulum.py for an example that does not show real-time values.
    Also see single_pendulum_simple for a version just shows the pendulum with no timing
    display or millisecond count.
...
win        = pybox.new_window("Pybox Pendulum (Timer Version)",size=(500,400),realtime=True)
...
#
# center_x    -- centers the text in the X plane
# pad_y(-20)  -- This option raises the text Y position -20 pixels 
